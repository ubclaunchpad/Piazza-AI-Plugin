...
async def create_chat_session(
    session_data: ChatSessionCreate, user=Depends(get_current_user)
):
    """
    ...
    try:
        ...
        thread_result = execute_query(
            thread_query, (session_data.piazza_course_id,), fetch_one=True
        )
        ...
        new_session = execute_query(
            insert_query,
            (user.id, thread_id, session_data.piazza_course_id, session_data.title),
            fetch_one=True,
        )
...
async def update_chat_session(
    session_id: UUID, update_data: ChatSessionUpdate, user=Depends(get_current_user)
):
    """
    ...
    try:
        query = """
            ...
            RETURNING id, user_id, thread_id, piazza_course_id, title, created_at, updated_at
        """
        updated_session = execute_query(
            query, (update_data.title, str(session_id), user.id), fetch_one=True
        )
...
async def get_chat_session_messages(session_id: UUID, user=Depends(get_current_user)):
    """
...
async def get_daily_query_count(user=Depends(get_current_user)):
    """
