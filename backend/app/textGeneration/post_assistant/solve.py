...
def stream_llm_solve_response(post_num: int, thread_id: str, session_id: str):
    """stream llm response in chunks"""
    message = (
        "Temporary solve response placeholder.\n\n"
        f"Post: {post_num}\n"
        f"Course: {thread_id}\n"
    ...
    try:
        ...
        history.add_user_message(
            f"[solve] Request for post {post_num} in course {thread_id}."
        )
        history.add_message(
            AIMessage(content=message, response_metadata={"sources": [str(post_num)]})
        )
    except Exception as e:
        logger.error(f"Failed to persist solve placeholder history: {e}")
