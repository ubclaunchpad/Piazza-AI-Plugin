...
def stream_placeholder_post_response(
    *,
    action_name: str,
    post_context: ResolvedPostContext,
    session_id: str | None,
    ...
    """Stream a placeholder response while still persisting history and sources."""
    sources = [str(post_context.post_num)]
    ...
    def _event_stream():
        ...
        logger.info(
            "Returning placeholder response for action=%s course=%s post=%s",
            action_name,
            post_context.thread_id,
            post_context.post_num,
        )
        yield json.dumps({"type": "content", "content": placeholder_message}) + "\n"
        yield json.dumps({"type": "sources", "sources": sources}) + "\n"
