...
class PostProcessor:
    ...
    def extract_metadata(post: Dict[str, Any]) -> PostMetadata:
        ...
        has_student_answer = any(child.get("type") == "s_answer" for child in children)

        # Count followups
        num_followups = sum(1 for child in children if child.get("type") == "followup")
    ...
    @staticmethod
    def process_answer(
        answer: Dict[str, Any], post_metadata: PostMetadata, answer_type: str
    ) -> Optional[PostChunk]:
        """
        ...
        answer_label = (
            "Instructor's Answer"
            if answer_type == "instructor_answer"
            else "Student's Answer"
        )
        text_parts = [
            f"## {answer_label}",
            "",
        ...
        if answer.get("endorsed", False):
            text_parts.insert(1, "**✓ Endorsed Answer**")
            text_parts.insert(2, "")
    ...
    def process_followups(
        children: List[Dict[str, Any]], post_metadata: PostMetadata
    ) -> List[PostChunk]:
        """
    ...
    def process_post(post: Dict[str, Any]) -> List[PostChunk]:
        ...
        for idx, response in enumerate(instructor_responses, 1):
            ...
            if clean_content:
                ...
                text_parts.append(
                    f"## Instructor Response {idx if len(instructor_responses) > 1 else ''}"
                )
                if endorsed:
        ...
        for idx, response in enumerate(student_responses, 1):
            ...
            if clean_content:
                endorsed = "✓ Endorsed " if response.get("endorsed", False) else ""
                text_parts.append(
                    f"## Student Response {idx if len(student_responses) > 1 else ''}"
                )
                if endorsed:
                    text_parts.append(f"**{endorsed}**")
                text_parts.append(clean_content)
                ...
                if replies:
                    for reply_idx, reply in enumerate(replies, 1):
                        ...
                        if reply_content:
                            ...
                            if role:
                                role_label = (
                                    f"**{role.capitalize()} Reply {reply_idx}:**"
                                )
                            else:
